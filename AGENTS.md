...
2. **Incremental updates** (`updater.rs`): PostgreSQL NOTIFY → debounce window → query affected features → re-encode MVT → write MBTiles
3. **Publishing** (`storage.rs`): copy/upload updated MBTiles artifact to local path, S3, or custom command backend
