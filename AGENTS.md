...
- **`metrics.rs`** — Prometheus registry (counters, gauges, histograms) and text exposition format, plus a standalone exporter for commands with no tile server. Process-wide singleton via `metrics()`; labels must stay low-cardinality.
